...
const LOGIN = "LOGIN"
const PRIVATEUNREADMSG = "PRIVATEUNREADMSG"
...
app.post('/registeruser', async function (req, res) {
    ...
    if (newemail) {
        ...
        user = await User.find({ useremail: newemail })
        data = user[0]
    }
        ...
        //     uuid: uuidv4()
        // })
        await User.updateOne({ mobileno: newmobile }, { $set: { username: req.body.username, uuid: uuidv4() } }, { upsert: true })
        user = await User.find({ useremail: newemail })
        data = user[0]
    }
...
io.on('connection', async (socket) => {
    ...
    // let user = new userDetails({ name: loginname, userid: loginid });
    // user.save();
    ...
    socket.on(ONETOONE, async (data) => {
        ...
        if (typeof participantd !== "undefined" ) {
            for (let i = 0; i < participantd.length; i++) {
                let d = 'D_' + participantd[i].userid
                dummy.push(d)
            }
    ...
    socket.on(GROUPJOIN, (data) => {
        ...
        ParticipantData.updateOne({ "channel": data.groupid, "userid": loginid }, { "$set": { "lastseen": new Date() } }, (err, updElem) => {
            if (err) {
                return ({ error: 'error' });
            }
            else {
               return ("updElem" + JSON.stringify(updElem));
    ...
    socket.on(LEAVEID, (data) => {
        ...
        socket.join('D_' + data.loginid)
        socket.leave(data.groupid)

        ParticipantData.updateOne({ "channel": data.groupid, "userid": data.loginid }, { "$set": { "lastseen": new Date() } }, (err, updElem) => {
            if (err) {
    ...
    socket.on(MESSAGE, async (message) => {
        ...
        if (message.type == 'private') {
            ...
            let clients = io.sockets.adapter.rooms.get(message.receiverid)

            const numClients = clients ? clients.size : 0;

            if (numClients == 1) {

                let str = message.receiverid
                var ret = str.replace(userid, '');
                var dummy = ret.replace('_', '');

                io.to('D_' + dummy).emit(PRIVATEUNREADMSG, message)
                io.to(message.receiverid).emit(MESSAGE, message)
            }
            ...
            let chatMessage = new Chat({ message: messagebody, senderid: sender, receiverid: receiver, name: loginname,type:message.type });
            chatMessage.save();
    ...
    socket.on("disconnect", () => {
        ...
        ParticipantData.updateOne({ "channel": socket.group, "userid": loginid }, { "$set": { "lastseen": new Date() } }, (err, updElem) => {
            if (err) {
                return ({ error: 'error in deleting address' });
            }
            else {
        ...
        GroupChat.countDocuments({ _id: data.groupid, ownerid: data.owner, name: data.name }, function (err, count) {
            ...
            if (count > 0) {
                participant.forEach(function (element) {
                    let metadata = new ParticipantData({ channel: data.groupid, userid: element.socketid, name: element.username, groupname: data.name, type: 'public' })
                    metadata.save()
                })
                var noti = {
    ...
    socket.on(REMOVEPARTICIPANT, (data) => {
        ...
        if (data.participant.length > 0) {
            ...
            GroupChat.countDocuments({ ownerid: data.owner, _id: data.id }, function (err, count) {
                ...
                if (count > 0) {
                    ParticipantData.remove({ channel: data.id, userid: { $in: naos } }, (err, result) => {
    ...
    socket.on(LEAVEGROUP, (data) => {
        ...
        GroupChat.countDocuments({ ownerid: data.owner, _id: data.id }, function (err, count) {
            if (count == 0) {
                ParticipantData.remove({ channel: data.id, userid: data.owner }, (err, updElem) => {
    ...
    socket.on(NEWADMIN, (data) => {
        ...
        GroupChat.updateOne({ _id: data.id }, { $set: { ownerid: data.ownerid } }, function (err, updElem) {
            ...
            ParticipantData.remove({ channel: data.id, userid: data.removeid }, (err, result) => {
...
app.post('/removeandmakeadmin', function (req, res) {
    ParticipantData.find({ channel: req.body.groupid }, (err, data) => {
        if (err) {
            res.status(401).json({ err });
        }
        else {
...
app.post('/deletegroup', async (req, res) => {
    ...
    const dele = await ParticipantData.deleteOne({ channel: req.body.groupid })
